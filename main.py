...
class DictationApp:
    ...
    def _cleanup_live_session(
        ...
        """Close the Live socket, then apply the single call's authoritative final.

        The streamed draft is interim ASR; the finalized smart ``input_transcription``
        lands at turn end (i.e. at stop). Swap the draft for that final when it
        differs; keep the draft when no final arrived.
        """
        try:
            live_text = session.stop(timeout=4.0, final_wait_s=2.5)
            ...
            if final_text and final_text != frozen:
                self._set_live_caret_desired(final_text)
                self._flush_live_caret(timeout=2.0)
                print(
                    f'>>> Live draft replaced with the smart final ("{final_text[:80]}").',
                    flush=True,
                )
            else:
                print(
    ...
    def _finish_live_session(
        ...
        try:
            # Single API call: give the Live final up to 2.5s to land before
            # any fallback — the smart final is model-generated, not instant.
            live_text = session.stop(timeout=4.0, final_wait_s=2.5)
            if epoch != self._live_epoch:
                return
            with self._live_caret_lock:
